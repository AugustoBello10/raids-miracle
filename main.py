...
app = Flask('')
@app.route('/')
def home(): return "Bot Bellão V38 (GOLD) Online"
...
class DynamicCraftingModal(ui.Modal):
    def __init__(self, item_name, receita_data, lang):
        ...
        # Pede preços dos ingredientes para calcular custo total (FEATURE V22 RESTAURADA)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            emb = discord.Embed(title=t['result_title'].format(qtd, self.item_name), color=discord.Color.blue())
            emb.add_field(name=t['chance'], value=f"{res['chance_sucesso']}%", inline=True)
            if res['custo_total'] > 0:
                emb.add_field(name=t['cost'], value=f"{res['custo_total']:,.0f} gp", inline=True)
            
            msg_list = ""
            for m, q in res['materiais_necessarios'].items():
                msg_list += f"• **{m}**: {q:.1f}\n"
            emb.add_field(name=t['list'], value=msg_list, inline=False)
            
            await interaction.response.send_message(embed=emb, ephemeral=True)
        except Exception as e: 
            await interaction.response.send_message(f"❌ Erro: {e}", ephemeral=True)

# --- SKILLS (V22 - MODAL DETALHADO) ---
class SingleSkillModal(ui.Modal):
    def __init__(self, voc, tipo, nome, spd, chg, lang):
        ...
        self.add_item(ui.TextInput(label="Preço da Arma (Opcional)", required=False)) # FEATURE V22
...
class ModeSelect(ui.View):
    ...
    async def rashid(self, i, b):
        ...
        emb = discord.Embed(title=f"Rashid: {info['city']}", description=info['desc'], color=discord.Color.gold())
...
class CategorySelect(ui.Select):
    def __init__(self, lang):
        self.lang=lang; t=TEXTOS[lang]; opts=[discord.SelectOption(label=t['cats'].get(k, k), value=k) for k in ESTRUTURA_MENU['crafting'].keys()]
...
class ItemSelect(ui.Select):
    ...
    async def callback(self, i):
        if self.values[0] not in RECEITAS: await i.response.send_message("❌ Erro.", ephemeral=True); return
