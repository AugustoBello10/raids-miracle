...
def calcular_mining(skill, pick_name):
    ...
    final_break = min(base_break + pick_stats['bonus_break'], 100.0)
    final_min = (2 + (0.1 * skill)) * (1 + pick_stats['bonus_collect'])
    final_frag = (0.5 + (0.025 * skill)) * (1 + pick_stats['bonus_collect'])
    return { "break_chance": round(final_break, 2), "minerals_chance": round(final_min, 2), "fragments_chance": round(final_frag, 2), "pick_used": pick_name }
